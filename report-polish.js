(function(){
  ...
  function refreshGrowthSelector(){
    const select=document.getElementById('growthTestCode');
    if(!select||typeof GROWTH_TESTS==='undefined')return;
    const current=select.value;
    select.innerHTML='<option value="">선택 안 함</option>'+GROWTH_TEST_ORDER.map(code=>`<option value="${code}">${escapeHtml(code)} · ${escapeHtml(GROWTH_TESTS[code].stage)} · ${escapeHtml(GROWTH_TESTS[code].gradeLevel||'')}</option>`).join('');
    if(current&&GROWTH_TESTS[current])select.value=current;
  }

  function keepGrowthFieldVisible(){
    const field=document.getElementById('growthTestField');
    if(field)field.classList.remove('hidden');
  }

  function installMiddleGrowthSupport(){
    refreshGrowthSelector();
    keepGrowthFieldVisible();

    document.querySelectorAll('input[name="schoolType"]').forEach(radio=>radio.addEventListener('change',()=>setTimeout(()=>{
      keepGrowthFieldVisible();
      refreshGrowthSelector();
    ...
    if(typeof originalSave==='function'&&!window.__middleGrowthSaveInstalled){
      window.__middleGrowthSaveInstalled=true;
