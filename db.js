...
async function connectFirebase(showMessage=true){
  try{
    ...
    alert('Firebase 연결 오류: '+(err.message||String(err)));
  }
}
...
async function loadEvaluationHistory(studentId=state.selectedStudentId){
  if(!studentId||!state.db){el('evaluationHistory').innerHTML='<option value="">저장된 평가가 없습니다</option>';return}
